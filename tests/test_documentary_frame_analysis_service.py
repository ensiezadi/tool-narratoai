...
""".strip()
    ...
    def test_export_for_kaggle_packages_full_video_task(self):
        service = DocumentaryFrameAnalysisService()

        with TemporaryDirectory() as temp_root:
            video_path = os.path.join(temp_root, "demo.mp4")
            with open(video_path, "wb") as fp:
                fp.write(b"fake-video")

            with patch("app.services.documentary.frame_analysis_service.utils.temp_dir", return_value=temp_root):
                result = service.export_for_kaggle(
                    video_path=video_path,
                    video_theme="demo theme",
                    custom_prompt="focus on actions",
                    frame_interval_input=3,
                    batch_size=4,
                )

            self.assertTrue(os.path.exists(result["local_zip_path"]))
            with zipfile.ZipFile(result["local_zip_path"], "r") as archive:
                names = archive.namelist()
                self.assertTrue(any(name.endswith("task_config.json") for name in names))
                self.assertTrue(any(name.endswith("input/demo.mp4") for name in names))
                self.assertTrue(any(name.endswith("narratoai_kaggle_video_runner.py") for name in names))
                self.assertFalse(any("/keyframes/" in name for name in names))

                config_name = next(name for name in names if name.endswith("task_config.json"))
                task_config = json.loads(archive.read(config_name).decode("utf-8"))
                self.assertEqual("video_frame_analysis", task_config["task_type"])
                self.assertEqual("input/demo.mp4", task_config["video_file"])
                self.assertEqual(3.0, task_config["frame_interval_seconds"])
                self.assertEqual(4, task_config["batch_size"])

    def test_import_kaggle_results_accepts_video_runner_artifact(self):
        service = DocumentaryFrameAnalysisService()
        ...
        with TemporaryDirectory() as temp_root:
            ...
            results_path = os.path.join(temp_root, "analysis_result.json")
            with open(results_path, "w", encoding="utf-8") as fp:
                json.dump(
                    {
                        "artifact_version": "narratoai-kaggle-video-v1",
                        "batches": [
                            {
                                "batch_index": 0,
                                "status": "success",
                                ...
                                "raw_response": "{}",
                                "frame_observations": [
                                    {"timestamp": "00:00:00,000", "observation": "第一帧"}
                                ],
                                "overall_activity_summary": "角色进入场景",
                            }
                        ],
                    },
                    fp,
            ...
            with patch.object(
                ...
                return_value=[os.path.join(temp_root, "keyframe_000000_000000000.jpg")],
            ):
            ...
            self.assertEqual(1, len(batch_results))
            self.assertEqual("success", batch_results[0].status)
            self.assertEqual("角色进入场景", batch_results[0].overall_activity_summary)
            self.assertEqual("第一帧", batch_results[0].frame_observations[0]["observation"])
