...
def render_video_config(tr, params):
    ...
    saved_aspect = st.session_state.get('video_aspect', VideoAspect.portrait.value)
    selected_aspect_index = 0
    for i, (_, value) in enumerate(video_aspect_ratios):
        if value == saved_aspect:
            selected_aspect_index = i
            break
    ...
    params.original_volume = st.slider(
        ...
        value=st.session_state.get('original_volume', AudioVolumeDefaults.ORIGINAL_VOLUME),
        step=0.01,
