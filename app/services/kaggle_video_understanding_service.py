...
from app.services.edit_decision import format_timestamp_range, parse_timestamp_range
...
class KaggleVideoUnderstandingService:
        ...
        if completed.returncode != 0:
            ...
            if "401" in output or "unauthorized" in output.lower():
                raise RuntimeError(
                    ...
                    "KAGGLE_USERNAME/KAGGLE_KEY 或 ~/.kaggle/kaggle.json"
                )
    ...
    def build_dataset_task(
        ...
        *,
        ...
        if max_analysis_seconds is not None and max_analysis_seconds <= 0:
            raise ValueError("max_analysis_seconds must be > 0")
        ...
        if max_analysis_seconds is not None:
            task_config["max_analysis_seconds"] = float(max_analysis_seconds)
        if analysis_ranges_seconds:
            task_config["analysis_ranges_seconds"] = analysis_ranges_seconds
    ...
    def run_on_kaggle(
        ...
        *,
        ...
        supports_push_accelerator = False
        if accelerator:
            help_result = _run_cli(["kaggle", "kernels", "push", "--help"], check=False)
            help_text = f"{help_result.stdout}\n{help_result.stderr}"
            supports_push_accelerator = "--accelerator" in help_text
        if accelerator and supports_push_accelerator:
            push_command.extend(["--accelerator", accelerator])
        elif accelerator:
            logger.info("当前 Kaggle CLI 不支持 kernels push --accelerator，改用 kernel-metadata.json 的 GPU 设置")
    ...
    def candidate_clips_to_script(
        ...
        *,
        ...
        compact_highlights: bool = False,
        target_duration_seconds: float | None = None,
        max_clips: int = 30,
        max_overlap_seconds: float = 2.0,
        require_narration: bool = False,
        narration_ost: int | None = None,
        stable_narration: bool = False,
    ) -> dict[str, Any]:
        ...
        target_duration_seconds = self._resolve_target_duration(payload, target_duration_seconds)
        selected = []
        for clip in clips:
            ...
            if parsed_range is None:
                continue
            narration = str(clip.get("narration_hint") or clip.get("narration") or "").strip()
            if require_narration and not narration:
                continue
        ...
        if compact_highlights:
            selected = self._compact_candidate_clips(
                selected,
            ...
            selected.sort(key=lambda item: (item["start"], -item["score"]))

        script = []
        used_narrations: set[str] = set()
        for index, item in enumerate(selected, start=1):
            clip = item["clip"]
            timestamp = format_timestamp_range(float(item["start"]), float(item["end"]))
            narration = item["narration"]
            if stable_narration:
                narration = self._stable_game_narration(clip, narration, used_narrations)
            used_narrations.add(narration)
            ost = int(clip.get("OST", 1) or 1)
            if narration and narration_ost in (0, 2):
                ost = narration_ost
            ...
            script.append(
                {
                    ...
                    "OST": ost,
                    "score": item["score"],
                    "event_type": str(clip.get("role") or clip.get("event_type") or ""),
                    "visual_evidence": str(clip.get("visual_evidence") or ""),
        ...
        return {
            ...
            "target_duration_seconds": target_duration_seconds,
            "compact_highlights": compact_highlights,
        }

    @staticmethod
    def _stable_game_narration(
        clip: dict[str, Any],
        narration: str,
        used_narrations: set[str] | None = None,
    ) -> str:
        """Keep game commentary short and grounded instead of hype-heavy."""
        ...
        if len(text) <= 34 and len(text) >= 12:
            return text + "。"

        event_type = str(clip.get("role") or clip.get("event_type") or "").strip()
        evidence = str(clip.get("visual_evidence") or clip.get("picture") or "")
        has_laser = "激光" in evidence
        has_barrage = "弹幕" in evidence
        has_explosion = "爆炸" in evidence or "火光" in evidence or "火球" in evidence
        has_boss = "Boss" in evidence or "BOSS" in evidence or "首领" in evidence
        has_black_white = "黑白" in evidence
        used_narrations = used_narrations or set()

        candidates: list[str] = []

        if event_type == "death_fail":
            if has_black_white:
                candidates.append("《双影奇境》的机关不等人，一步失误就是爆炸收场。")
            candidates.append("这次处理慢了一拍，爆炸直接终结挑战。")
        elif event_type == "puzzle_progress":
            if has_laser and has_explosion:
                candidates.append("贴着红色激光找路线，最后一击刚好引爆机关。")
            candidates.append("关键机关被成功触发，路线终于打开。")
        elif event_type == "strong_reaction":
            if "必殺技" in evidence or has_black_white:
                candidates.append("《双影奇境》这段刚收必杀，机械巨口立刻压场。")
            if "绿色球体" in evidence:
                candidates.append("双人刚穿过激光网，核心爆炸把节奏顶上来。")
            if "CRITICAL" in evidence or "暴击" in evidence:
                candidates.append("暴击提示连续跳出，Boss 在火球里彻底散架。")
            if has_boss and has_explosion:
                candidates.append("这段协作输出很关键，机械首领开始被打崩。")
            if has_laser and has_barrage:
                candidates.append("两边火力同时压中心，强光爆炸完成收尾。")
            if has_explosion:
                candidates.append("爆炸在画面中心炸开，节奏被瞬间推高。")

        if 12 <= len(text) <= 34:
            candidates.append(text + "。")
        candidates.append(text[:34].rstrip("，,。") + "。")

        for candidate in candidates:
            candidate = candidate.strip()
            if candidate and candidate not in used_narrations:
                return candidate
        return (candidates[0] if candidates else "关键画面出现变化，节奏在这里明显抬升。").strip()

    def _compact_candidate_clips(
        self,
        ...
        *,
        ...
        """Pick a short, high-signal timeline from dense benchmark candidates."""
        if max_clips <= 0:
            return []
        ...
        ranked = sorted(
            ...
            key=lambda item: (
                -float(item["score"]),
                -self._event_priority(item["clip"]),
                float(item["start"]),
            ),
        )
        picked: list[dict[str, Any]] = []
        picked_keys: set[tuple[int, int, str]] = set()
        total_duration = 0.0
        ...
        for item in ranked:
            ...
            dedupe_key = (
                round(start * 2),
                round(end * 2),
                str(clip.get("storyboard") or clip.get("source_event_ids") or clip.get("clip_id") or ""),
            )
            if dedupe_key in picked_keys:
                continue

            if any(self._overlap_seconds(start, end, float(chosen["start"]), float(chosen["end"])) > max_overlap_seconds for chosen in picked):
                continue

            if picked and total_duration + duration > target_duration_seconds:
                continue

            picked.append(item)
            picked_keys.add(dedupe_key)
            total_duration += duration

            if len(picked) >= max_clips or total_duration >= target_duration_seconds:
                break

        if not picked and selected:
            picked = [max(selected, key=lambda item: float(item["score"]))]

        picked.sort(key=lambda item: (float(item["start"]), -float(item["score"])))
        logger.info(
            "Kaggle 候选片段精选完成: "
            f"{len(selected)} -> {len(picked)}, total_duration={sum(float(i['end']) - float(i['start']) for i in picked):.1f}s"
        )
        return picked
    ...
    @staticmethod
    def _event_priority(clip: dict[str, Any]) -> float:
        event_type = str(clip.get("role") or clip.get("event_type") or "").strip()
        priorities = {
            "death_fail": 4.0,
            "strong_reaction": 3.0,
    ...
    @staticmethod
    def _resolve_target_duration(payload: Any, target_duration_seconds: float | None) -> float:
    ...
    def _prepare_kernel_notebook(
        ...
        if overrides:
            ...
            preconfig_index = next(
                (
                    index for index, cell in enumerate(notebook.get("cells", []))
                    if "configs = []" in "".join(cell.get("source", []))
                ),
                len(notebook.get("cells", [])),
            )
            notebook["cells"].insert(preconfig_index, preconfig_override_cell)
            ...
            override_cell = {
                ...
                "source": [
                    ...
                    "print('Applied runtime overrides:', runtime_overrides)\n",
                ],
            }
            config_index = next(
                (
